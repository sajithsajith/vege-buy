...
def fifth():
    price_list = [
        values["brinjal"] * price[0],
        values["cabbage"] * price[1],
        values["green_banana"] * price[2],
        values["lemon"] * price[3],
        values["tomato"] * price[4],
    ]
    return render_template(
        "result.html",
        brinjal=round(values["brinjal"], 2),
        cabbage=round(values["cabbage"], 2),
        green_banana=round(values["green_banana"], 2),
        lemon=round(values["lemon"], 2),
        tomato=round(values["tomato"], 2),
        total_weight=round(sum(values.values()), 2),
        brinjal_price=round(price_list[0], 2),
        cabbage_price=round(price_list[1], 2),
        green_banana_price=round(price_list[2], 2),
        lemon_price=round(price_list[3], 2),
        tomato_price=round(price_list[4], 2),
...
def video_feed():
    return Response(
        capture(), mimetype="multipart/x-mixed-replace; boundary=frame"
    )
